...
class TrainPre(object):
    ...
    def __call__(self, img, pred, gt, fgprob):
        ...
        cv2.imshow('pred', (p_pred*255).squeeze().astype(np.uint8))
        cv2.imshow('gt', (p_gt*255).squeeze().astype(np.uint8))
        cv2.imshow('prob', (p_fgprob*255).squeeze().astype(np.uint8))
        cv2.waitKey()
...
def get_train_loader(engine, dataset):
    ...
    train_dataset = dataset(data_setting, "train", 0.5, train_preprocess,
